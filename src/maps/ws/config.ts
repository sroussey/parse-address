...
/**
 * Samoa (WS) address configuration.
 *
 * Language: English. Order: house NUMBER first, then the street name + a
 * TRAILING English type suffix ("14 Beach Road"). Type echoed verbatim.
 * Postcode: NONE. Samoa has no postal-code system, so the postal slot never matches and the place tail ends at the city (and optional island).
 * Region: the island/region (Upolu, Savai'i, Manono ...) is the final comma segment and maps to `state`.
 * PO Box is the dominant address form in Samoa; "PO Box", "P.O. Box",
...
export const wsConfig: EuCountryConfig = {
  code: "ws",
  country: "WS",
  countryNames: ["Western Samoa", "Samoa", "WSM", "WS"],
