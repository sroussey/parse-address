...
/**
 * Kiribati (KI) address configuration.
 *
 * Language: English. Order: house NUMBER first, then the street name + a
 * TRAILING English type suffix ("3 Bairiki Road"). Type echoed verbatim.
 * Postcode: NONE. Kiribati has no postal-code system, so the postal slot never matches and the place tail ends at the city (and optional island).
 * Region: the island/region (South Tarawa, North Tarawa, Tarawa ...) is the final comma segment and maps to `state`.
 * PO Box is the dominant address form in Kiribati; "PO Box", "P.O. Box",
...
export const kiConfig: EuCountryConfig = {
  code: "ki",
  country: "KI",
  countryNames: ["Kiribati", "KIR", "KI"],
