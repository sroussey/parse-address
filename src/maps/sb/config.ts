...
/**
 * Solomon Islands (SB) address configuration.
 *
 * Language: English. Order: house NUMBER first, then the street name + a
 * TRAILING English type suffix ("5 Mendana Avenue"). Type echoed verbatim.
 * Postcode: NONE. Solomon Islands has no postal-code system, so the postal slot never matches and the place tail ends at the city (and optional island).
 * Region: the island/region (Guadalcanal, Malaita, Western ...) is the final comma segment and maps to `state`.
 * PO Box is the dominant address form in Solomon Islands; "PO Box", "P.O. Box",
...
export const sbConfig: EuCountryConfig = {
  code: "sb",
  country: "SB",
  countryNames: ["Solomon Islands", "SLB", "SB"],
