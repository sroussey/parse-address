...
/**
 * Vanuatu (VU) address configuration.
 *
 * Language: English. Order: house NUMBER first, then the street name + a
 * TRAILING English type suffix ("8 Kumul Highway"). Type echoed verbatim.
 * Postcode: NONE. Vanuatu has no postal-code system, so the postal slot never matches and the place tail ends at the city (and optional island).
 * Region: the island/region (Espiritu Santo, Santo, Efate ...) is the final comma segment and maps to `state`.
 * PO Box is the dominant address form in Vanuatu; "PO Box", "P.O. Box",
...
export const vuConfig: EuCountryConfig = {
  code: "vu",
  country: "VU",
  countryNames: ["Vanuatu", "VUT", "VU"],
