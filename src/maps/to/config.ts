...
/**
 * Tonga (TO) address configuration.
 *
 * Language: English. Order: house NUMBER first, then the street name + a
 * TRAILING English type suffix ("12 Taufa'ahau Road"). Type echoed verbatim.
 * Postcode: NONE. Tonga has no postal-code system, so the postal slot never matches and the place tail ends at the city (and optional island).
 * Region: the island/region (Tongatapu, Vava'u, Ha'apai ...) is the final comma segment and maps to `state`.
 * PO Box is the dominant address form in Tonga; "PO Box", "P.O. Box",
...
export const toConfig: EuCountryConfig = {
  code: "to",
  country: "TO",
  countryNames: ["Kingdom of Tonga", "Tonga", "TON", "TO"],
