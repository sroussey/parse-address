# Saint-Pierre-et-Miquelon (PM) — address research

**Grammar = FR (metropolitan France) grammar, verbatim.** Saint-Pierre-et-Miquelon is a French
...
- **Postcode:** 5 digits, prefix `975` (pattern `975\d\d`), written
  BEFORE the commune. `CEDEX nn` markers after the city are dropped.
- **country field:** `PM`.
- **No region/state** — `countyPattern` set to never-match `(?!x)x`.
- **Sample localities:** Saint-Pierre, Miquelon, Langlade, Miquelon-Langlade, Île-aux-Marins.
