# Martinique (MQ) — address research

**Grammar = FR (metropolitan France) grammar, verbatim.** Martinique is a French
...
- **Postcode:** 5 digits, prefix `972` (pattern `972\d\d`), written
  BEFORE the commune. `CEDEX nn` markers after the city are dropped.
- **country field:** `MQ`.
- **No region/state** — `countyPattern` set to never-match `(?!x)x`.
- **Sample localities:** Fort-de-France, Le Lamentin, Schoelcher, Sainte-Marie, Le Robert, Saint-Pierre.
