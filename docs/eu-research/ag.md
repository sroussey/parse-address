# Antigua and Barbuda (AG) — address research

## Sources / basis
- Universal Postal Union (UPU) addressing guidelines and S42 country pages.
- ISO 3166-2:AG administrative subdivisions (parishes / dependencies / islands).
...
- **PO Box:** very common — `P.O. Box 123, St. John's, ...`. Plain numeric.
- **Postcode:** No nationwide postcode in everyday use. A never-match sentinel is used; the place tail is city + parish/island.
- **State (region):** the parish / island written as the last comma segment maps to `state`: Saint John, Saint George, Saint Mary, Saint Paul, Saint Peter, Saint Philip, Barbuda, Redonda, Antigua. The county pattern is restricted to this list so a trailing country name (`Antigua and Barbuda`) is never captured as a region.
- **Capital / principal settlement:** St. John's.

## Country name spellings accepted
Antigua and Barbuda, Antigua & Barbuda, ATG, AG
