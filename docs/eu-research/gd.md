# Grenada (GD) — address research

## Sources / basis
- Universal Postal Union (UPU) addressing guidelines and S42 country pages.
- ISO 3166-2:GD administrative subdivisions (parishes / dependencies / islands).
...
- **PO Box:** very common — `P.O. Box 123, St. George's, ...`. Plain numeric.
- **Postcode:** No nationwide postcode in everyday use. A never-match sentinel is used; the place tail is city + parish/island.
- **State (region):** the parish / island written as the last comma segment maps to `state`: Saint Andrew, Saint David, Saint George, Saint John, Saint Mark, Saint Patrick, Carriacou. The county pattern is restricted to this list so a trailing country name (`Grenada`) is never captured as a region.
- **Capital / principal settlement:** St. George's.
