# Saint Vincent and the Grenadines (VC) — address research

## Sources / basis
- Universal Postal Union (UPU) addressing guidelines and S42 country pages.
- ISO 3166-2:VC administrative subdivisions (parishes / dependencies / islands).
...
- **PO Box:** very common — `P.O. Box 123, Kingstown, ...`. Plain numeric.
- **Postcode:** No nationwide postcode in everyday use. A never-match sentinel is used; the place tail is city + parish/island.
- **State (region):** the parish / island written as the last comma segment maps to `state`: St. Vincent, Bequia, Union Island, Canouan, Mustique, Mayreau. The county pattern is restricted to this list so a trailing country name (`St. Vincent and the Grenadines`) is never captured as a region.
- **Capital / principal settlement:** Kingstown.

## Country name spellings accepted
Saint Vincent and the Grenadines, St. Vincent and the Grenadines, St Vincent and the Grenadines, VCT, VC
