# La Réunion (RE) — address research

**Grammar = FR (metropolitan France) grammar, verbatim.** La Réunion is a French
...
- **Postcode:** 5 digits, prefix `974` (pattern `974\d\d`), written
  BEFORE the commune. `CEDEX nn` markers after the city are dropped.
- **country field:** `RE`.
- **No region/state** — `countyPattern` set to never-match `(?!x)x`.
- **Sample localities:** Saint-Denis, Saint-Paul, Saint-Pierre, Le Tampon, Saint-André, Saint-Louis.
